...
// AZURE
router.post('/azure/create/virtualmachine', jsonParser, (
  req,
  response,
) => {
...
router.post('/azure/create/virtualnetwork', jsonParser, (
  req,
  response,
) => {
  if (utils.validateUser(req.body.token)) {
...
router.post('/azure/create/database', jsonParser, (req, response) => {
  if (utils.validateUser(req.body.token)) {
    const { body } = req;
    const resourceGroupName = body['Resource group'];
    ...
    sql
      ...
      .then((res) => sql.createOrUpdateDatabase(
        resourceGroupName,
        serverName,
        databaseName,
        createDBParams,
...
// AMAZON WEB SERVICES
router.post('/aws/create/virtualmachine', jsonParser, (req, response) => {
  if (utils.validateUser(req.body.token)) {
    const { body } = req;
    if (body.awsType === 'EC2') {
      const params = {
...
router.post('/aws/create/virtualnetwork', jsonParser, (req, response) => {
  if (utils.validateUser(req.body.token)) {
    const { body } = req;
  } else {
    res = {
...
router.post('/aws/create/database', jsonParser, (req, response) => {
  if (utils.validateUser(req.body.token)) {
    const { body } = req;
...
router.post('/aws/create/security', jsonParser, (req, response) => {
  if (utils.validateUser(req.body.token)) {
    const { body } = req;
  } else {
    res = {
...
router.post('/gcp/create/virtualmachine', jsonParser, (req, response) => {
  if (utils.validateUser(req.body.token)) {
    const { body } = req;
  } else {
    res = {
...
router.post('/gcp/create/virtualnetwork', jsonParser, (req, response) => {
  if (utils.validateUser(req.body.token)) {
    const { body } = req;
  } else {
    res = {
...
router.post('/gcp/create/database', jsonParser, (req, response) => {
  if (utils.validateUser(req.body.token)) {
    const { body } = req;
  } else {
    res = {
