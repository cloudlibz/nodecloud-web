...
Seeder.prototype._waterfallBatch = function (seeds) {
  ...
  (0, _lodash.each)(seeds, seed => {
    ...
    current = current.then(() => // Nesting promise to prevent bubbling up of error on catch
    _bluebird.default.resolve().then(() => seed.seed(knex, _bluebird.default)).then(() => log.push(name)).catch(originalError => {
